...
class TransformerDataSet(Dataset):
    def __init__(self, root_directory, image_directory=None, segmentation_directory=None, image_fragment=None, segmentation_fragment=None, image_extension=".png", segmentation_extension=".png", dataset_limit=None):
        images = []
        annotations = []
        file_names = []
        ...
        segmentation_path = os.path.join(root_directory, segmentation_directory)
        ...
        for file_name in tqdm(files, desc="Loading dataset"):
            ...
            if os.path.isfile(file_path) and file_path.lower().endswith(('png', 'jpg', 'jpeg', 'bmp', 'tiff', 'gif')):
                segmentation_file_name = file_name.replace(image_fragment, segmentation_fragment).replace(image_extension, segmentation_extension)
                segmentation_file_path = os.path.join(segmentation_path, segmentation_file_name)
                if os.path.exists(segmentation_file_path):
                    image = Image.open(file_path).convert('RGB')
                    images.append(image)
                    segmentation = Image.open(segmentation_file_path).convert('L')
    ...
    def __getitem__(self, index):
        ...
        sample = {"image": self.images[index], "annotation": self.annotations[index], "filename": self.filenames[index]}
        return sample
    ...
    @staticmethod
    def train_val_dataset(dataset, validation_split=0.30, training_split=0.70, random_state=None, shuffle=True):
...
class ImageSegmentationDataset(Dataset):
    def __init__(self, dataset, image_processor, image_transformations=None, ignore_index=255):
    ...
    def __getitem__(self, index):
        ...
        if annotation_array.shape != image_array.shape[:2]:
            raise ValueError(f"Mask and image shapes do not match at index {index}: {annotation_array.shape} vs {image_array.shape[:2]}")
        
        # Apply transformations if provided
        if self.transform is not None:
            transformed = self.transform(image=image_array, mask=annotation_array)
            image_array, annotation_array = transformed["image"], transformed["mask"]
        ...
        for label in class_labels:
            ...
            instance_to_class_mapping.update({instance_id: label for instance_id in instance_ids})
        ...
        inputs = self.processor([image_array], [annotation_array], instance_id_to_semantic_id=instance_to_class_mapping, return_tensors="pt")
        inputs = {key: value.squeeze() if isinstance(value, torch.Tensor) else value[0] for key, value in inputs.items()}
        return inputs
...
def collate_fn(batch_samples):
    ...
    pixel_mask = torch.stack([sample["pixel_mask"] for sample in batch_samples])
...
def create_loss_plots(train_loss_values, validation_loss_values, timestamp, save_directory):
    ...
    axs[1].plot(list(range(len(validation_loss_values))), validation_loss_values)
    axs[1].set_xlabel('Epochs')
    axs[1].set_ylabel('Loss')
    axs[1].set_title('Validation Loss')

    plt.subplots_adjust(hspace=0.5)
    plt.savefig(os.path.join(save_directory, f'Train_and_Val_loss_{timestamp}.png'), dpi=300)
    
def visualize_instance_seg_mask(segmentation_mask, label_to_color_mapping, mask_type='groundtruth'):
    ...
    segmentation_mask = np.where(segmentation_mask == -1, 255, segmentation_mask)
    image = np.zeros((segmentation_mask.shape[0], segmentation_mask.shape[1], 3), dtype=np.uint8)
    ...
    for height in range(image.shape[0]):
        for width in range(image.shape[1]):
            ...
            if color_mask in label_to_color_mapping:
                ...
                print(f'{color_mask} is not an index in the colormap. Using unknown color.')
                image[height, width, :] = (219, 52, 235)
    
    return image / 255.0

def get_inference(dataset_sample, processor, device, model, image_extension, threshold=None):
    ...
    # Post-process the output based on the threshold
    ...
    # Extract the instance segmentation mask
    instance_segmentation_mask = result["segmentation"].cpu().detach().numpy()

    # Extract confidence scores
    if "scores" in result:
        confidence_scores = result["scores"].cpu().detach().numpy()
        predicted_classes = np.argmax(confidence_scores, axis=-1)
        confidences = np.max(confidence_scores, axis=-1)
    else:
        if hasattr(outputs, 'class_probabilities'):
            class_probabilities = outputs.class_probabilities
            confidences = class_probabilities.max(dim=1)[0].cpu().numpy()
        else:
            confidences = np.ones_like(instance_segmentation_mask)

    return instance_segmentation_mask, confidences
...
def create_image_fromdataset(index, dataset, save_directory, timestamp):
    ...
    for plot_index in range(2):
        ...
        plt.title(title)
        plt.axis("off")
    plt.savefig(os.path.join(save_directory, f"Image_seg_{index}_{timestamp}.png"), dpi=300)

print("Compiled Methods")

# Load user configuration from YAML file
with open("/home/lhernandez2/Venus_SAR/Maskformer/user_config_train.yaml", "r") as file:
...
IMGDIR = usr_config['IMGDIR']
IFRAG = usr_config['IFRAG']
SEGDIR = usr_config['SEGDIR']
...
NUM_CLASSES = len(ID2LABEL)  # Ensure this matches the number of classes in your dataset
...
if __name__ == "__main__":
    ...
    tf_dataset = TransformerDataSet(DATADIR, IMGDIR, SEGDIR, IFRAG, SFRAG, IMFIX, LABFIX, dataset_limit=LIMIT_DATASET)
    ...
    # Split the dataset into training and validation sets
    training_val_dataset = tf_dataset.train_val_dataset(tf_dataset, random_state=RANDOM_SEED)
    ...
    config = MaskFormerConfig.from_pretrained(local_model_dir, num_labels=NUM_CLASSES, local_files_only=True)
    model = MaskFormerForInstanceSegmentation.from_pretrained(local_model_dir, config=config, local_files_only=True, ignore_mismatched_sizes=True)
    ...
    base_model = MaskFormerModel.from_pretrained(local_model_dir, local_files_only=True)
    ...
    train_dataset = ImageSegmentationDataset(
        ...
        image_transformations=train_val_transform
    )
    ...
    BATCH_SIZE = min(BATCH_SIZE, 2)  # Reduce batch size to 2 or the value in the configuration, whichever is smaller
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # Switch back to GPU
    model.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        # Training loop
        with tqdm(train_dataloader, desc=f"Training Epoch {epoch+1}/{num_epochs}") as train_pbar:
            for idx, batch in enumerate(train_pbar):
                ...
                try:
                    ...
                    if idx % 50 == 0:
                        train_pbar.set_postfix({"Training loss": round(sum(train_loss) / len(train_loss), 6)})
        ...
        # Validation loop
        with tqdm(val_dataloader, desc=f"Validation Epoch {epoch+1}/{num_epochs}") as val_pbar:
            for idx, batch in enumerate(val_pbar):
                try:
                    ...
                    with torch.no_grad():
                        ...
                        if idx % 50 == 0:
                            val_pbar.set_postfix({"Validation loss": round(sum(val_loss) / len(val_loss), 6)})

                except RuntimeError as e:
                    print(f"RuntimeError at validation batch {idx}: {e}")
                    for key, value in batch.items():
                        if isinstance(value, torch.Tensor):
                            print(f"Validation batch key: {key}, shape: {value.shape}, device: {value.device}")
                        else:
                            print(f"Validation batch key: {key}, length: {len(value)}")
                    raise
        
        # Compute average validation loss for the epoch
        avg_val_loss = sum(val_loss) / len(val_loss)
        avg_val.append(avg_val_loss)
        print(f"Epoch {epoch+1}/{num_epochs} | train_loss: {avg_train_loss} | validation_loss: {avg_val_loss}")
